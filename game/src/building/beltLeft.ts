...
export class ConveyorBeltLeft implements Building {
  private static readonly baseTile = new Vec2(0, 3);
  ...
  drawReal(renderer: Renderer, s: number) {
    renderer.drawSprite(this.pos.mul(renderer.tileSize), ConveyorBeltLeft.baseTile);
  }

  public drawGhost(renderer: Renderer, pos: Vec2, _params: AllBuildingParams) {
    renderer.drawSprite(pos.mul(renderer.tileSize), ConveyorBeltLeft.baseTile);
  }
