...
export class ConveyorBeltDown implements Building {
  ...
  drawReal(renderer: Renderer, s: number) {
    renderer.drawSprite(this.pos.mul(renderer.tileSize), ConveyorBeltDown.baseTile);
  }

  public drawGhost(renderer: Renderer, pos: Vec2, _params: AllBuildingParams) {
    renderer.drawSprite(pos.mul(renderer.tileSize), ConveyorBeltDown.baseTile);
  }
